  ...
  app.post('/api/project/addproject' , (req, res) => {   //新增專案
    ...
    const sql = "INSERT INTO project ( `username`, `projectName` ) VALUES (?)";   //新增專案的命令
    const values = [
      req.body.username,
  ...
  app.get('/api/project/getproject', (req, res) => {   //查詢指定使用者的所有專案
    const sql = "SELECT * From project WHERE username = (?)";
  ...
  app.post('/api/project/deleteproject', ( req, res ) => {   //刪除指定使用者的指定專案
    const sql = "DELETE FROM project WHERE username = (?) AND projectName = (?)";
    const values = [
      req.body.username,
      req.body.projectName
    ]
    db.query( sql, [values[0], values[1]], ( err ) => {
      if(err){
  ...
  app.get('/api/project/getstep', ( req, res) => {   //不明
    const sql = "SELECT * FROM user WHERE username = (?) AND projectName = (?)";
    const values = [
      req.body.username,
      req.body.projectName
    ]
